...
export function updateOptions(
    ...
    const newOptions = [...options]; // Create a copy to maintain immutability
    if (targetOptionIndex === -1) {
        newOptions.push(newOption); // Append to the end
    } else if (
        targetOptionIndex >= 0 &&
        targetOptionIndex < newOptions.length
    ) {
        newOptions[targetOptionIndex] = newOption; // Replace existing option
    }
    return newOptions;
}
